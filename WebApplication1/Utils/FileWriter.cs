...
namespace ProfApreciat.Utils
{
    public class FileWriter
    {
        ...
        public static void work(List<Profesor> results, int nrAbs)
        {
            ...
            using (XLWorkbook workbook = new XLWorkbook(destFilePath))
            {
                ...
                for (int rowIndex = 4; rowIndex < rows.Count; rowIndex++)
                {
                    ...
                    var currResult = results[resultIndex];
                    ...
                    int totalVoturi = nrVoturiLicenta + nrVoturiMaster;
                    float procent = (totalVoturi * 1.0f) / nrAbs;
                ...
                for (int rowNr = 5; rowNr <= rows.Count; rowNr++)
                {
                    ...
                    if (remuneratiFaza1[results[resultIndex].FacultateServiciu].Contains(results[resultIndex].ID_Profesor))
                    {
                        ws.Column(10).Cell(rowNr).Value = "DA";
                        ws.Column(10).Cell(rowNr).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);
                    }
                    
                    if (remuneratiFaza2[results[resultIndex].FacultateServiciu].Contains(results[resultIndex].ID_Profesor))
                    {
